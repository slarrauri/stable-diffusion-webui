...
ENTRYPOINT ["python", "launch.py", "--no-half", "--use-cpu all", "--listen", "--data-dir", "/app/data", "--disable-console-progressbars", "--enable-insecure-extension-access"]
CMD ["--skip-torch-cuda-test","--opt-sdp-no-mem-attention","--opt-channelslast"]
